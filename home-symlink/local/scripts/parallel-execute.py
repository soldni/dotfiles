...
def parse_options():
    ...
    ap.add_argument(
        '-a', '--path-as-arg', default=False, action='store_true',
        help="If true, pass the path as argument"
    )
    ap.add_argument(
        '-c', '--cpu-count', type=int, default=(multiprocessing.cpu_count() - 1)
    )
...
def driver():
    ...
    paths = [
        os.path.join(opts.directory, fn)
        for fn in os.listdir(opts.directory)
    ]
    ...
    process_args = [
        '{c}{a}{o} {r}'.format(
            ...
            o=' '.join(
                '{} {}'.format(opt, fp)
                for opt in opts.options.strip().split()
            ),
